        ...
        if ($result) {
           echo"<tr><th> Name</th> <th>education</th> <th>email</th> <th>PHONE</th> <th>ADDRESS</th> <th> USER_ID</th><th>Action</th></tr> ";
            while ($row = mysqli_fetch_assoc($result)) {
                $uid = $row["user_id"];
                echo "<tr> <td>" . $row["full_name"] . "</td> <td>" . $row["edu"] . "</td> <td>" . $row["email"] . "</td> <td>" . $row["phone"] . "</td> <td>" . $row["address"] . "</td> <td>" . $uid  . "</td><td>.<a href='student_remove.php?user_id_de=" . $uid . "'>Remove</a></td>" . "</tr><br>";
                echo "";
            }
        } else {
